...
namespace SerialRGBSample
{
    class WaveConvert
    {
        ...
        private static double _Wavelength = 580;
        private static bool _Dir = false;

        public static Color GetNextRainbowColor()
        {
            if (_Wavelength >= 775)
                _Dir = true;
            else if (_Wavelength <= 385)
                _Dir = false;
            _ = _Dir ? _Wavelength -= 4 : _Wavelength += 4;
            int[] vals = waveLengthToRGB(_Wavelength);
